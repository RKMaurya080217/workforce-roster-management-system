...
@Entity
@DiscriminatorValue("ROSTER_CHANGE")
public class RosterChangeRequest extends EmployeeWorkflowRequest {
    ...
    @Column(name = "current_weekly_off")
    private Boolean currentWeeklyOff = false;

    @Enumerated(EnumType.STRING)
    @Column(name = "requested_shift_type")
    private ShiftType requestedShiftType;

    @Column(name = "requested_weekly_off")
    private Boolean requestedWeeklyOff = false;

    @Column(name = "request_reason", length = 1000)
    private String reason;

    @Enumerated(EnumType.STRING)
    @Column(name = "roster_change_status")
    private RosterChangeStatus status = RosterChangeStatus.PENDING;

    @Column(name = "roster_admin_remarks", length = 1000)
    private String adminRemarks;

    @Column(name = "roster_created_at")
    ...
    public boolean isCurrentWeeklyOff() { return currentWeeklyOff != null && currentWeeklyOff; }
    public Boolean getCurrentWeeklyOff() { return currentWeeklyOff; }
    ...
    public boolean isRequestedWeeklyOff() { return requestedWeeklyOff != null && requestedWeeklyOff; }
    public Boolean getRequestedWeeklyOff() { return requestedWeeklyOff; }
